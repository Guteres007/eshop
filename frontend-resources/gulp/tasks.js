...
exports.makeVendorPipe = function (config) {
    const tasks = [
        gulp
            ...
            .pipe(
                gulpRename((path) => (path.dirname = `vendor/${path.dirname}`))
            ),
    ];

    for (let source in config.vendorModules) {
        if (config.vendorModules.hasOwnProperty(source)) {
            const dest = config.vendorModules[source];
            ...
            tasks.push(
                gulp
                    ...
                    .pipe(
                        gulpRename(
                            (path) => (path.dirname = `${dest}/${path.dirname}`)
                        )
                    )
            );
        }
    ...
    return mergeStream(...tasks).pipe(
        gulpIf(!config.pack, gulp.dest(config.distDir))
    );
};
...
exports.makeTasks = function (config) {
    ...
    // tasks
    const sassTask = function sass() {
        return exports.makeSassPipe(config).pipe(server.stream());
    };
    const vendorTask = function vendor() {
        return exports.makeVendorPipe(config);
    };
    const svgTask = function svg() {
        return exports.makeSvgPipe(config);
    };
    const imagesTask = function images() {
        return exports.makeImagesPipe(config);
    };
    const jsTask = function js() {
        return exports.makeJsPipe(config);
    };
    const fontsTask = function fonts() {
        return exports.makeFontsPipe(config);
    };
    ...
    const vendorWatch = config.vendorWatch.slice();

    for (let source in config.vendorModules) {
        if (config.vendorModules.hasOwnProperty(source)) {
            vendorWatch.push(source);
        }
    }
    ...
    const watchTask = function () {
        ...
        gulp.watch(
            vendorWatch,
            { ignoreInitial: false },
            gulp.series(vendorTask, reloadTask)
        );
        gulp.watch(
            config.svgWatch,
            { ignoreInitial: false },
            gulp.series(svgTask, reloadTask)
        );
        gulp.watch(
            config.imagesWatch,
            { ignoreInitial: false },
            gulp.series(imagesTask, reloadTask)
        );
        gulp.watch(
            config.jsWatch,
            { ignoreInitial: false },
            gulp.series(jsTask, reloadTask)
        );
        gulp.watch(
            config.fontsWatch,
            { ignoreInitial: false },
            gulp.series(fontsTask, reloadTask)
        );
    };
     ...
     */
    const serveTask = gulp.series(cleanTask, startServerTask, watchTask);
